...
class IAM(DataModuleBase):
    ...
    def __init__(self, args: argparse.Namespace = None):
        super().__init__(args)
        self.metadata = toml.load(self._get_data_meta_path())

    def prepare_data(self, *args, **kwargs) -> None:
        if self.xml_filenames:
            return
        filename = self._download_raw_dataset(self.metadata, self._get_data_dl_dir())  # type: ignore
        self._extract_raw_dataset(filename, self._get_data_dl_dir())

    @property
    def xml_filenames(self):
        dir_extract = self._get_data_extract_dir("iamdb")
        return list(Path(dir_extract, "xml").glob("*.xml"))

    @property
    def form_filenames(self):
        dir_extract = self._get_data_extract_dir("iamdb")
        return list(Path(dir_extract, "forms").glob("*.jpg"))
    ...
    def line_strings_by_id(self):
        ...
        return {
            filename.stem: self._get_line_strings_from_xml_file(filename)
    ...
    def line_regions_by_id(self):
        ...
        return {
            filename.stem: self._get_line_regions_from_xml_file(filename)
            for filename in self.xml_filenames
        }
    ...
    @classmethod
    def _extract_raw_dataset(cls, filename: Path, dirname: Path) -> None:
    ...
    @classmethod
    def _get_line_strings_from_xml_file(cls, filename: str) -> List[str]:
    ...
    @classmethod
    def _get_line_regions_from_xml_file(cls, filename: str) -> List[Dict[str, int]]:
        """Get the line region dict for each line."""
        xml_root_element = ElementTree.parse(filename).getroot()  # nosec
        xml_line_elements = xml_root_element.findall("handwritten-part/line")
        return [cls._get_line_region_from_xml_element(el) for el in xml_line_elements]

    @classmethod
    def _get_line_region_from_xml_element(cls, xml_line) -> Dict[str, int]:
        """
        Parameters
        ----------
        xml_line
...
if __name__ == "__main__":
    torch_io.load_and_print_info(IAM)
