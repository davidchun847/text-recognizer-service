...
class EMNISTLines2(DataModuleBase):
    ...
    def setup(self, stage: str = None) -> None:
        ...
        if stage == "fit" or stage is None:
            ...
            self.data_train = DatasetBase(
                x_train, y_train, transform=self.get_transform(augment=self.augment)
            )
            self.data_val = DatasetBase(
                x_val, y_val, transform=self.get_transform(augment=self.augment)
            )
        ...
        if stage == "test" or stage is None:
            ...
            self.data_test = DatasetBase(
                x_test, y_test, transform=self.get_transform(augment=False)
            )
    ...
    def create_dataset_of_images(
        ...
        for n in range(N):
            ...
            crop = cls.construct_image_from_string(
    ...
    @classmethod
    def get_transform(cls, augment=False):
...
if __name__ == "__main__":
    torch_io.load_and_print_info(EMNISTLines2)
