...
class ResnetTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.start_token = inverse_mapping[str_utils.get_special_char(str_utils.ESpecialChar.start)]
        self.end_token = inverse_mapping[str_utils.get_special_char(str_utils.ESpecialChar.end)]
        self.padding_token = inverse_mapping[str_utils.get_special_char(str_utils.ESpecialChar.pad)]
    ...
    def predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for Sy in range(1, S):
            ...
            output_tokens[:, Sy : Sy + 1] = torch.reshape(output[-1:], (-1, 1))  # Set the last output token
